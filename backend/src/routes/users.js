...
router.get('/me', authenticateToken, async (req, res) => {
  const user = await db.prepare('SELECT id, full_name, username, email, role, is_active, banned_until, created_at FROM users WHERE id = ?').get(req.user.id);
  res.json({ user });
});
...
router.get('/:id', authenticateToken, async (req, res) => {
  ...
  const user = await db.prepare('SELECT id, full_name, username, email, role, is_active, banned_until, created_at FROM users WHERE id = ?').get(req.params.id);
...
router.put('/:id', authenticateToken, async (req, res) => {
  ...
  await db.prepare(`UPDATE users SET full_name=?, is_active=?, role=?, banned_until=?, updated_at=datetime('now') WHERE id=?`)
    .run(newName, newActive, newRole, newBannedUntil, req.params.id);
  await db.prepare(`INSERT INTO activity_logs (user_id, action, details) VALUES (?, 'user_update', ?)`).run(req.user.id, `Usuario ${req.params.id} actualizado`);
  const updated = await db.prepare('SELECT id, full_name, username, email, role, is_active, banned_until, created_at FROM users WHERE id = ?').get(req.params.id);
...
router.delete('/:id', authenticateToken, requireAdmin, async (req, res) => {
  ...
  const activeLoans = (await db.prepare(`SELECT COUNT(*) as c FROM loans WHERE user_id = ? AND status IN ('reserved','active','overdue')`).get(req.params.id)).c;
  if (activeLoans > 0) return res.status(409).json({ error: 'El usuario tiene préstamos activos y no puede ser eliminado' });

  try {
    // Delete related
    await db.prepare('DELETE FROM messages WHERE sender_id = ? OR receiver_id = ?').run(req.params.id, req.params.id);
    await db.prepare('DELETE FROM notifications WHERE user_id = ?').run(req.params.id);
    await db.prepare('DELETE FROM loans WHERE user_id = ?').run(req.params.id);
    
    await db.prepare('DELETE FROM users WHERE id = ?').run(req.params.id);
    await db.prepare(`INSERT INTO activity_logs (user_id, action, details) VALUES (?, 'user_delete', ?)`).run(req.user.id, `Usuario ${user.username} eliminado`);
